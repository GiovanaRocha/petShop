...
class EmpresaDAO{
	...
	public function logar($login,$password){
		include("conexao.php");
		$sql = 'SELECT * FROM empresa WHERE login = :login and senha = :password';
	...
	public function inserir($empresa){
		include("conexao.php");
		$sql = 'INSERT INTO empresa (estado, bairro, cidade, cep, rua, numero, logo, imgAdc, imgLogo, cnpj, login, senha, nome) VALUES (:estado, :bairro, :cidade, :cep, :rua, :numero,:logo, :imgAdc, :imgLogo, :cnpj, :login, :senha, :nome)';
		$consulta = $conexao->prepare($sql);
	...
	public function atualizar($empresa){
		include("conexao.php");
		$sql = 'UPDATE empresa SET estado = :estado, bairro = :bairro, cidade = :cidade, cep = :cep, rua = :rua, numero = :numero, logo = :logo, imgAdc = :imgAdc, imgLogo = :imgLogo, cnpj = :cnpj, login = :login, senha = :senha, nome = :nome WHERE cod = :cod';
		$consulta = $conexao->prepare($sql);
		$consulta->bindValue(':estado',$empresa->getEstado()); 
		$consulta->bindValue(':bairro',$empresa->getBairro()); 
		$consulta->bindValue(':cidade',$empresa->getCidade()); 
